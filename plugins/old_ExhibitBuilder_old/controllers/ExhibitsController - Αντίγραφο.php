...
class ExhibitBuilder_ExhibitsController extends Omeka_Controller_Action
{
    ...
    public function browseAction()
    {
		...
		$count=0;
		$user = current_user();
		foreach( $records as $key=>$record ): //echo $record['id']."<br>";
		if($user['id']==1 or $user['id']==11 or $record->wasAddedBy(current_user())){
		$count+=1;
		...
		$totalrecords = $table->findAll();
		$count=0;
		foreach( $totalrecords as $keytotal=>$totalrecord ): //echo $record['id']."<br>";
		if($user['id']==1 or $user['id']==11 or $totalrecord->wasAddedBy(current_user())){
		$count+=1;
