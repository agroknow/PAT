...
if ($exhibitSection->title) {
    $exhibitSectionTitle = __('Edit Section: \"%s\"', __($exhibitSection->title));
} else {
    $exhibitSectionTitle = ($actionName == 'Add') ? __('Add Section') : __('Edit Section');
}
$exhibitSectionTitle=stripslashes($exhibitSectionTitle);
?>
<?php head(array('title'=> html_escape(stripslashes($exhibitSectionTitle)), 'bodyclass'=>'exhibits')); ?>
...
<form method="post" accept-charset="utf-8" action="" id="section-metadata-form" class="exhibit-builder">
	...
	<fieldset>
	    ...
	    <div class="field"><?php echo text(array('name'=>'title', 'id'=>'title', 'class'=>'textinput','readonly'=>'readonly'), __($exhibitSection->title), __('Title')); ?></div>
        <div class="field">
            <?php echo hidden(array('name'=>'slug','id'=>'slug','class'=>'textinput','readonly'=>'readonly'), $exhibitSection->slug, __('Slug')); ?>
            <p class="explanation"><?php //echo __('No spaces or special characters allowed.'); ?></p>
        </div>
    	<?php /*<div class="field"><?php echo textarea(array('name'=>'description', 'id'=>'description', 'class'=>'textinput','rows'=>'10','cols'=>'40'), $exhibitSection->description, __('Description')); ?></div>*/ ?>
	</fieldset>
