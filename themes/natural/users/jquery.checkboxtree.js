(function(jQuery) {
	jQuery.fn.checkboxTree = function(settings) {
		...
		var $group = this;
		var $labels = jQuery(":checkbox + span",$group)
		...
		$group.bind("click.checkboxtree",function(e,a){
			...
			var $clickedparentlabel = $clickedparent.next("span");
			var clickedlabel = false;
			var clickedimage = false;
			if($clicked.is(":checkbox + span")){
				clickedlabel=true;
			}
			if($clicked.is("img.checkboxtreeimage")){
				clickedimage=true;
			}
			//when the span is clicked, set the checkbox to the opposite clicked state, and toggle the checked class
			if(clickedlabel){
				...
				if(settings.checkchildren&&a!=="checkparents"){
					$currrow.find(":checkbox + span").prev().attr({checked: $clickedcheck.attr("checked")?"checked":""})
					.next()
		...
		jQuery("input:checked + span",$group).addClass(settings.checkedClass)
