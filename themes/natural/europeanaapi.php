...
<div>Filter results by type: 
  <script>
  function GoType(){document.form4.submit();}
  </script>
...
<select name="bytype">
<option onclick="GoType();" value="">NONE</option>
<option onclick="GoType();" value="IMAGE">IMAGE</option>
<option onclick="GoType();" value="TEXT">TEXT</option>
<option onclick="GoType();" value="VIDEO">VIDEO</option>
<option onclick="GoType();" value="SOUND">SOUND</option>
</select>
</form>
</div>
<?php 
libxml_use_internal_errors(false);
$xml = simplexml_load_file('http://api.europeana.eu/api/opensearch.rss?searchTerms='.$europeanatext.''.$europenana_type.'&startPage='.$startPage.'&wskey=IIRTOOIRNG', NULL, LIBXML_NOERROR | LIBXML_NOWARNING);
...
$europenana = $child2->children('http://www.europeana.eu');
  if(strlen($europenana->rights)>7){print "Rights : <a href='".$europenana->rights."' target='_blank'>".$europenana->rights."</a><br />";} 
