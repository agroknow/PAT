...
$query_item="select * from omeka_exhibits where public=1";
...
$metadatarecord_res=mysql_query($sqlmetadatarecord);
$metadatarecord=mysql_fetch_array($metadatarecord_res);
...
$output .= '<record>'."\n";
	$output .= '<header>'."\n";
		$output .= '<identifier>';
			$output .=  'oai:'.$_SERVER['SERVER_NAME'].':'.$metadatarecord['id'];		
		$output .= '</identifier>'."\n";
		$output .= '<datestamp>';
			$output .=  $row_item['date_modified'];		
		$output .= '</datestamp>'."\n";
	$output .= '</header>'."\n"; 
	$output .= '<metadata>'."\n";
		$output .= '<lom xmlns="http://ltsc.ieee.org/xsd/LOM">'."\n";
					...
					while($metadatarecordvalue=mysql_fetch_array($metadatarecordvalue_res)){
							...
							$sql2="select * from metadata_element_hierarchy where id=".$metadatarecordvalue['element_hierarchy']."";
							$sql_res2=mysql_query($sql2);
							$result2=mysql_fetch_array($sql_res2);
							
							$sql2="select * from metadata_element_label where id=".$result2['element_id']."";
							$sql_res2=mysql_query($sql2);
							$result3=mysql_fetch_array($sql_res2);
