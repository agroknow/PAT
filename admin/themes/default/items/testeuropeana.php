...
//echo 'http://collections.natural-europe.eu:8081/cmss/search?query='.$europeanatext.'&start='.$startPageurl.'';
?>
<div><em><strong>Filter results by type: </strong></em><br> 
  ...
  <?php if(isset($_POST['dataProvider'])){$dataProvider=$_POST['dataProvider'];}else{$dataProvider='';} ?>
...
$xml = simplexml_load_file('http://collections.natural-europe.eu:8081/cmss/search?query='.$bytypeforurl.''.$dataProviderforurl.'text:'.$europeanatext.'&start='.$startPageurl.'', NULL, LIBXML_NOERROR | LIBXML_NOWARNING);
echo 'http://collections.natural-europe.eu:8081/cmss/search?query='.$bytypeforurl.''.$dataProviderforurl.'text:'.$europeanatext.'&start='.$startPageurl.'';
...
$opensearch = $child1->children('http://a9.com/-/spec/opensearch/1.1/');
 ...
 // print "<br />startIndex : ".$opensearch->startIndex;
  ...
  <?php if(isset($_POST['dataProvider'])){?> <input type="hidden" name="dataProvider" value="<?php echo $_POST['dataProvider']; ?>"> <?php } ?>
  ...
  ?>
	...
	<br />
		...
		<span style="text-decoration:underline; position:relative; top:20px; font-weight:bold;">PROVIDERS</span><br>
		<?php echo ingest_search_providers(''.$europeanatext.'','nhmc'); ?>
        <?php echo ingest_search_providers(''.$europeanatext.'','mnhn'); ?>
        <?php echo ingest_search_providers(''.$europeanatext.'','hnhm'); ?>
        <?php echo ingest_search_providers(''.$europeanatext.'','tnhm'); ?>
        <?php echo ingest_search_providers(''.$europeanatext.'','ac'); ?>
        <?php echo ingest_search_providers(''.$europeanatext.'','jme'); ?>
		</div>
	</div> 
