...
<p id="add-item" class="add-button" style="position:relative; right:10px; top:0px;"><a class="add" id="displayText" href="<?php echo html_escape(uri('items/ingestitemtorepository')); ?>"><?php echo __('Ingest a Resource'); ?></a></p>
...
<form id="items-browse" action="<?php echo html_escape(uri('items/batch-edit')); ?>" method="post" accept-charset="utf-8">
    ...
    <tr class="item <?php if(++$key%2==1) echo 'odd'; else echo 'even'; ?>">
        ...
        <td class="item-info">
            ...
            <?php /*<li><?php $uri=WEB_ROOT; $uri=explode('http://',$uri);	echo '<a href="http://'.$uri[1].'/oai?verb=GetRecordOnlyLom&metadataPrefix=oai_lom&identifier=oai:'.$uri[1].'/:'.$item->id.':item" target="_blank">XML</a>'; ?></li>*/ ?>
...
//} 
        ...
        <td><?php echo ($typeName = __(item('Item Type Name')))
                    ? __($typeName)
                    : '<em>' . __(item('Dublin Core', 'Type', array('snippet' => 35))) . '</em>'; ?></td>
        <td>
        <?php if($item->public): ?>
