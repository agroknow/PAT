...
//$fp = fopen("/var/www/html/".uri('archive/xmlvoc')."/new_oe_ontology_hierrarchy.xml","w");
            ...
            //echo "/var/www/" . uri('') . "/" . $xmlname . ".xml";
            //$fp = file_put_contents("C:/Program Files (x86)/EasyPHP-12.1/www/" . uri('') . "/" . $xmlname . ".xml", fopen("" . $newout . "", 'r'));
            $fp = file_put_contents("/var/www/" . $xmlname . ".xml", fopen("" . $newout . "", 'r'));
            if ($fp) {
                echo "<a href='http://" . $_SERVER['SERVER_NAME'] . "/" . $xmlname . ".xml' target='_blank'> New Xml from Europeana</a>";
            }
        }
        if ($all == 2 or $all == 3) {
            //echo "2"; die;
//create object for zip          
            $zip = new ZipArchive();
            $filename = "/var/www/testharvest/europeana-my-archive.zip";
...
//check if can open file
            ...
            foreach ($link as $link) {
                ...
                $fp = file_put_contents("/var/www/" . $xmlname . ".xml", fopen("" . $xmlfile . "", 'r'));
                //array_push($files_to_zip, "C:/Program Files (x86)/EasyPHP-12.1/www/" . uri('') . "/" . $xmlname . ".xml");
                array_push($files_to_zip, "/var/www/" . $xmlname . ".xml");
                if (!$fp == FALSE) {
                    //echo "<a href='http://" . $_SERVER['SERVER_NAME'] . "/" . $xmlname . ".xml' target='_blank'>".$xmlname."</a><br>";
                }
                //Add xml into the zip file
                $zip->addFile("/var/www/" . $xmlname . ".xml", $title . ".xml");
...
//            $destination = "/var/www/testharvest/europeana-my-archive.zip";
//            $result = create_zip($files_to_zip, $destination);
//            if (!$result == FALSE) {
//                echo "<a href='http://" . $_SERVER['SERVER_NAME'] . "/europeana-my-archive.zip' target='_blank'>Europeana ZIP</a><br>";
//            }
            echo "numfiles: " . $zip->numFiles . "\n";
            echo "status:" . $zip->status . "\n";
            $zip->close();
            echo "<a href='http://" . $_SERVER['SERVER_NAME'] . "/testharvest/europeana-my-archive.zip' target='_blank'>Europeana ZIP file</a><br>";
        }
    }
