...
//$title = mb_convert_encoding($title, "UTF-8"); //echo $title; break;
//$title = htmlspecialchars_decode($title);
$title=addslashes($title);
$description=addslashes($description);

$mainAttributesSql="INSERT INTO omeka_element_texts (record_id ,element_id,record_type_id,text) VALUES ('".$last_exhibit_id."',68,2,'".$title."')";
...
$metadatarecordSql="INSERT INTO metadata_element_value (element_hierarchy, value, language_id, multi, record_id) VALUES ('34','".$_POST['source']."','none',1, ".$last_record_id.")";
$execmetadatarecordSql=$db->query($metadatarecordSql);
$exec=null;
...
$metadatarecordSql="INSERT INTO metadata_element_value (element_hierarchy, value, language_id, multi, record_id, parent_indexer) VALUES ('6','".$title."','en',1, ".$last_record_id.",1)";
$execmetadatarecordSql=$db->query($metadatarecordSql);
$exec=null;

$metadatarecordSql="INSERT INTO metadata_element_value (element_hierarchy, value, language_id, multi, record_id, parent_indexer) VALUES ('8','".$description."','en',1, ".$last_record_id.",1)";
$execmetadatarecordSql=$db->query($metadatarecordSql);
$exec=null;
...
$record_id=$row["id"];
$exec=null;
if(isset($_POST['item_url'])){$_POST['55_1']=$_POST['item_url']; $_POST['32_1']=$_POST['item_url'];}
//print_r($_POST);break;
...
if($_POST['type']){$formtype=$_POST['type'];} else{$formtype="0";}
...
$uri=WEB_ROOT;	
  if(stripos($data51['value'],".jpg")>0 or stripos($data51['value'],".gif")>0 or $datarecordit['item_type_id']==6 or stripos($data51['value'],".jpeg")>0 or stripos($data51['value'],".png")>0 or stripos($data51['value'],".bmp")>0 or stripos($data51['value'],"content/thumbs/src")>0 or $dataformatfromvoc['value']=="IMAGE"){
		   ...
		   $string= '<img src="http://open.thumbshots.org/image.aspx?url='.$data51['value'].'" style=" width:'.$widththumb.'px; max-height:'.$widththumb.'px;"><br>';
...
$uri=WEB_ROOT;	
  if(stripos($data51['value'],".jpg")>0 or stripos($data51['value'],".gif")>0  or $datarecordit['item_type_id']==6 or stripos($data51['value'],".jpeg")>0 or stripos($data51['value'],".png")>0 or stripos($data51['value'],".bmp")>0 or stripos($data51['value'],"content/thumbs/src")>0 or $dataformatfromvoc['value']=="IMAGE"){
		   ...
		   $string= '<img src="http://open.thumbshots.org/image.aspx?url='.$data51['value'].'" style=" max-width:500px; max-height:400px; height:auto;"/><br>';
		   // echo ''.$data51['value'].'<br>';
...
/////////////////////////general//////////////////
if(isset($search_item)){$europeanatext= $search_item;
...
if(isset($_POST['dataProvider'])){$dataProviderforurl= "dataProvider:".$_POST['dataProvider']." AND ";} else{$dataProviderforurl="";}
...
$europeanatext_forcultural=urlencode($europeanatext);
            $resp = call_cultural_federation($europeanatext_forcultural);
...
<a href="#" onclick="submitform();" <?php if($search_type==1){ echo 'style=" font-weight:bold; text-decoration:underline;"';} ?>> Natural Europe (<?php echo $total_naturaleurope; ?>)</a><br>
</form>
<br>
...
<input type="hidden" name="europeanatext" value="<?php echo $europeanatext; ?>">
<?php if(isset($_POST['bytype'])){?> <input type="hidden" name="bytype" value="<?php echo $_POST['bytype']; ?>"> <?php } ?>
<a href="#" onclick="submitform2();" <?php if($search_type==2){ echo 'style=" font-weight:bold; text-decoration:underline;"';} ?>>Europeana (<?php echo $total_euroepana; ?>)  </a><br>
</form>
<br>
  ...
  <input type="hidden" name="startPage" value="1">
<input type="hidden" name="europeanatext" value="<?php echo $europeanatext; ?>">
<a href="#" onclick="submitform3();" <?php if($search_type==3){ echo 'style=" font-weight:bold; text-decoration:underline;"';} ?>>Ariadne (<?php echo $total_ariadne; ?>)  </a>
...
if(isset($_POST['dataProvider']) and $_POST['dataProvider']==$dataProvider){$search_type=1;}//echo $search_type;
//$dataProvider="nhmc";
if(isset($dataProvider)){$dataProviderforurl= "dataProvider:".$dataProvider." AND ";} else{$dataProviderforurl="";}
 $naturaleuropexml = simplexml_load_file('http://collections.natural-europe.eu/cmss/search?query='.$bytypeforurl.''.$dataProviderforurl.'text:'.$europeanatext.'&start='.$startPageurl.'', NULL, LIBXML_NOERROR | LIBXML_NOWARNING);
...
<a href="#" style="text-transform:uppercase;<?php if($search_type==1){ echo 'font-weight:bold; text-decoration:underline;';} ?>" onclick="document.<?php echo $dataProvider; ?>form.submit();return false;"> <?php echo $dataProvider; ?> (<?php echo $total_naturaleurope; ?>)</a>
